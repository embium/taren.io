<script lang="ts">
	...
	import { CreditCard, Calendar, Clock, AlertTriangle, CheckCircle2, XCircle, Zap, ChevronRight } from '@lucide/svelte';
...
<div class="flex flex-col h-full">
	...
	<!-- Content -->
	<div class="flex-1 overflow-y-auto px-4 sm:px-6 md:px-8 py-6">
		<div class="max-w-2xl mx-auto space-y-5">

			{#if loading}
				<!-- Skeleton -->
				<div class="rounded-xl border border-border bg-card p-6 animate-pulse space-y-4">
			...
			{:else if !subscription}
				<!-- No subscription -->
				<div class="rounded-xl border border-border bg-card p-10 text-center space-y-4">
					...
					<p class="text-sm text-muted-foreground">You are currently on the free plan. Upgrade to unlock full features.</p>
					<a
						href="/pricing"
			...
			{:else}
				<!-- Trial status banner -->
				{#if isTrialing}
					<div class="flex items-start gap-3 rounded-xl border border-amber-500/30 bg-amber-500/10 px-5 py-4">
						...
						<div>
							...
							<p class="mt-0.5 text-sm text-amber-400/80">
								Your trial ends on <span class="font-medium text-amber-300">{formatDate(subscription.trial_end)}</span>.
								You will be charged <span class="font-medium text-amber-300">{formatAmount(subscription.amount)}/{subscription.interval}</span> after the trial ends.
							</p>
						</div>
					</div>
				{:else if isCancelledAtEnd}
					<div class="flex items-start gap-3 rounded-xl border border-red-500/30 bg-red-500/10 px-5 py-4">
						...
						<div>
							...
							<p class="mt-0.5 text-sm text-red-400/80">
								Your subscription will end on <span class="font-medium text-red-300">{formatDate(subscription.current_period_end)}</span>.
				...
				<div class="rounded-xl border border-border bg-card overflow-hidden">
					...
					<div class="flex items-center justify-between px-6 py-5 border-b border-border">
						<div class="flex items-center gap-3">
							...
							<div>
								<p class="text-xs font-medium text-muted-foreground uppercase tracking-wide">Current Plan</p>
					...
					<div class="divide-y divide-border">
						...
						<div class="flex items-center justify-between px-6 py-4">
							...
							<span class="text-sm font-semibold">
								{#if isTrialing}
									<span class="text-emerald-400 font-bold mr-2">$0.00 now</span>
									<span class="text-muted-foreground">then {formatAmount(subscription.amount)}/{subscription.interval}</span>
								{:else}
						...
						{#if isTrialing && subscription.trial_end}
							<div class="flex items-center justify-between px-6 py-4">
								...
								<span class="text-sm font-semibold text-amber-400">{formatDate(subscription.trial_end)}</span>
							</div>
							<div class="flex items-center justify-between px-6 py-4">
								<div class="flex items-center gap-3">
									<Calendar class="h-4 w-4 text-muted-foreground" />
						...
						{:else}
							<div class="flex items-center justify-between px-6 py-4">
								...
								<span class="text-sm font-semibold {isCancelledAtEnd ? 'text-red-400' : ''}">{formatDate(subscription.current_period_end)}</span>
							</div>
						{/if}
					</div>
					...
					<!-- Cancel action -->
					{#if !isCancelledAtEnd}
						<div class="px-6 py-5 border-t border-border bg-muted/30">
							{#if !showCancelConfirm}
								<button
									id="cancel-subscription-btn"
									onclick={() => (showCancelConfirm = true)}
									class="text-sm text-muted-foreground hover:text-destructive transition-colors flex items-center gap-1.5"
								>
							...
							{:else}
								<div class="space-y-3">
									<p class="text-sm text-muted-foreground">
										...
										<span class="font-medium text-foreground">{formatDate(subscription.current_period_end)}</span>, then ends.
									</p>
									<div class="flex items-center gap-3">
										<button
											...
											disabled={cancelling}
											class="inline-flex items-center gap-2 rounded-lg border border-destructive/50 bg-destructive/10 px-4 py-2 text-sm font-medium text-destructive hover:bg-destructive/20 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
										>
											{#if cancelling}
				...
				{#if isTrialing}
					<div class="rounded-xl border border-indigo-500/20 bg-indigo-500/5 px-6 py-4 flex items-center justify-between gap-4">
						<div>
							<p class="text-sm font-medium">Enjoying Taren?</p>
							<p class="text-xs text-muted-foreground mt-0.5">Upgrade to Professional for more scans and advanced analytics.</p>
						</div>
						<a
							href="/pricing"
							class="inline-flex shrink-0 items-center gap-1.5 rounded-lg bg-indigo-600 px-4 py-2 text-sm font-semibold text-white hover:bg-indigo-500 transition-colors"
						>
							Upgrade
